#include <WiFi.h>
#include <ArduinoJson.h> //json format files
...
#define VARIABLE_LABEL_DISTANCE "distance"    // Label referente a variável de distancia criada no ubidots
...
bool sendValues(float distance){
  ...
  sprintf(json,  "{\"%s\":{\"value\":%02.02f, \"context\":{\"distance\":%02.02f}}}", VARIABLE_LABEL_DISTANCE, distance, distance);  
...
bool sendValues(String deviceId,float distance, int filterMode){ //filterMode: 0 no filter, 1 average, 2 median, 3 kalman
  
  String jsonMessageMQTT = "";
  StaticJsonDocument<300> jsonDoc;
  jsonDoc["deviceID"] = deviceId;
  jsonDoc["distance"] = String(distance);
  jsonDoc["filterMode"] = String(filterMode);
 
  serializeJson(jsonDoc, jsonMessageMQTT);
  

  if(!heltechClient.publish(TOPIC, jsonMessageMQTT.c_str(), true) )
    return false;
